...
{
	int a = 10;
	return a; // 不可以，尝试返回 a 的地址给引用变量，a是存在栈里的，函数结束调用栈被销毁。
}

int * func()
{
	int a = 10;
	return &a; // 不可以，尝试返回 a 的地址给指针，a是存在栈里的，函数结束调用栈被销毁。
}

int func()
{
	int a = 10;
	return a; // 可以，函数在返回的时候，返回处产生一个临时对象，用于存放局部变量a的值的一份拷贝（变量a的右值的拷贝）
}
...
为什么4.7.5 里面，基类的纯虚析构里面没有释放子类内存的操作

![1660200834819](assets/1660200834819.png)